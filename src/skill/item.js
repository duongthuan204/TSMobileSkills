...
class Skill extends React.Component {
    ...
    render() {
        const { type, skill, update } = this.props
        const { doublePoint } = this.state
        const imgUrl = "./assets/" + type + "/" + skill.id + ".png"
        return <div>
            <div className="skill-item" onClick={() => update(skill.id, doublePoint, type)} data-tip={this.renderTooltip()} data-multiline={true} data-effect="solid" data-delay-show="200">
                <img className={skill.point < 1 ? 'skill-inactived' : ''} src={imgUrl} width="50" height="50"></img>
